...
import { creditService, type CashSaleHistory, type CreditCustomer, type CreditNote, type CreditNoteWithStatus, type CreditPayment, type CreditPaymentEvidence, type CreditPaymentMethod, type CreditSummary, type CustomerAddress, type SalePaymentEvidence } from '../../services/concretera/credit.service';
import { concreteWalletService, type CustomerWalletMovement, type CustomerWalletSummary } from '../../services/concretera/wallet.service';
...
import { auditService, logConcreteraAudit } from '../../services/audit/audit.service';
...
const CustomerScreen: React.FC<CustomerScreenProps> = ({ selectedBranchId, branches, currentUser }) => {
  ...
  const loadCustomers = useCallback(async () => {
    ...
    try {
      const [pageData, branchWallets] = await Promise.all([
        ...
        concreteWalletService.listWalletsByBranch(branchId),
      ]);
  ...
  const handleSearchEligibleWalletCustomers = useCallback(async (query: string) => {
    if (!branchId) return;
    const data = await concreteWalletService.listEligibleCustomers(branchId, query);
  ...
  const handleCreateWallet = async (event: React.FormEvent) => {
    ...
    try {
      const wallet = await concreteWalletService.createWallet({
        branch_id: branchId,
  ...
  const handleRechargeWallet = async (event: React.FormEvent) => {
    ...
    try {
      await concreteWalletService.rechargeWallet({
      ...
      if (walletToHistory?.id === walletToRecharge.id) {
        ...
        const movements = await concreteWalletService.listWalletMovements(walletToRecharge.id);
  ...
  const openWalletHistoryModal = async (wallet: CustomerWalletSummary) => {
    ...
    setIsWalletHistoryLoading(true);
    try {
      const movements = await concreteWalletService.listWalletMovements(wallet.id);
      setWalletMovements(movements);
    } catch (err) {
  ...
  const loadPaymentHistory = async (customer: CreditCustomer) => {
    ...
    if (cashSales.length > 0) {
      ...
      const totals = (saleItems ?? []).reduce<Record<string, number>>((acc, item: any) => {
        ...
        acc[key] = (acc[key] ?? 0) + Number(item.line_total ?? (Number(item.qty ?? 0) * Number(item.unit_price ?? 0)));
        return acc;
      }, {});
  ...
  const inferSaleType = (row: any): 'Mayoreo' | 'Menudeo' | '—' => {
    ...
    const wholesale = Number(row.concrete_product_uoms?.wholesale_price ?? row.concrete_products?.wholesale_price ?? 0);
    const retail = Number(
      row.concrete_product_uoms?.retail_price
      ?? row.concrete_products?.retail_price
      ?? row.concrete_products?.precio
      ?? 0
    );
  ...
  const fetchSaleSummaryByTransactionId = async (transactionId: string, fallback?: {
    ...
    const { data: itemRows, error: itemsError } = await supabase
      ...
      .select(`
        ...
        line_total,
        factor_used,
        concrete_products ( name, sku, attrs, wholesale_price, retail_price, precio ),
        concrete_product_uoms ( uom_id, wholesale_price, retail_price, concrete_uoms ( name, code ) )
      `)
    ...
    const items = (itemRows ?? []).map((row: any) => {
      ...
      const item = {
        ...
        product_sku: row.concrete_products?.sku ?? null,
        uom_name: row.concrete_product_uoms?.concrete_uoms?.name ?? null,
        uom_code: row.concrete_product_uoms?.concrete_uoms?.code ?? null,
  ...
  const generateSalePdf = async (input: {
    ...
    const drawCellText = (page: PDFPage, text: string, col: number, y: number, size: number, color = rgb(0, 0, 0)) => {
    ...
    pages.forEach((pageItems, pageIndex) => {
      ...
      ['PRODUCTO', 'PRESENTACION', 'CANTIDAD', 'PRECIO UNITARIO', 'SUBTOTAL'].forEach((header, idx) => {
        drawCellText(page, header, idx, tableTop - 12, 8, rgb(1, 1, 1));
      });
      ...
      pageItems.forEach((item) => {
        ...
        drawCellText(page, item.name.toUpperCase(), 0, rowY + 4, 8);
        drawCellText(page, item.presentation.toUpperCase(), 1, rowY + 4, 8);
        drawCellText(page, Number(item.qty).toFixed(2), 2, rowY + 4, 8);
        drawCellText(page, formatCurrency(Number(item.unitPrice)), 3, rowY + 4, 8);
        drawCellText(page, formatCurrency(subtotal), 4, rowY + 4, 8);
        rowY -= rowHeight;
      });
      ...
      if (pageIndex === pages.length - 1) {
        ...
        if (payments.length > 0) {
          ...
          ['NO.', 'FECHA', 'METODO', 'MONTO'].forEach((header, index) => {
            page.drawText(header, { x: paymentCols[index] + 5, y: paymentY + 5, size: 7, font: fontBold, color: rgb(1, 1, 1) });
          });
          paymentY -= 16;
    ...
    <div className="space-y-6">
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden animate-in zoom-in flex flex-col">
      ...
      {isEditModalOpen && selectedCustomer && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden animate-in zoom-in flex flex-col">
            ...
            <form onSubmit={handleUpdateCustomer} className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nombre del cliente</label>
                <input
                  required
