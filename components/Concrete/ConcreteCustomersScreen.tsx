...
const CustomerScreen: React.FC<CustomerScreenProps> = ({ selectedBranchId, branches, currentUser }) => {
  ...
  const [noteSaleSummaries, setNoteSaleSummaries] = useState<Record<string, ConcreteSaleSummaryData>>({});
  ...
  const inferConcreteSaleType = (row: any): 'Mayoreo' | 'Menudeo' | '—' => {
    ...
    const retail = Number(row.concrete_product_uoms?.retail_price ?? row.concrete_products?.retail_price ?? row.concrete_products?.precio ?? 0);
  ...
  const toggleNoteSaleSummary = async (note: CreditNoteWithStatus) => {
    ...
    try {
      const summary = await fetchConcreteNoteSaleSummary(note);
  ...
  const handlePrintSale = async (note: CreditNoteWithStatus) => {
    ...
    try {
      const summary = noteSaleSummaries[note.id] ?? await fetchConcreteNoteSaleSummary(note);
      if (!noteSaleSummaries[note.id]) {
        setNoteSaleSummaries((prev) => ({ ...prev, [note.id]: summary }));
      }
            ...
            <div className="bg-slate-900 p-6 text-white flex justify-between items-start">
              <div>
                ...
                <p className="text-slate-300 font-bold tracking-widest uppercase text-[10px] mt-1">
                  {getPaymentNote(selectedPaymentForEvidence.note_id) ? getDisplayNoteCode(getPaymentNote(selectedPaymentForEvidence.note_id) as CreditNote) : 'Sin folio'} · {String(selectedPaymentForEvidence.paid_at ?? '').replace('T', ' ').slice(0, 16)}
                </p>
              </div>
              <button
                onClick={() => {
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-[11px] text-slate-400">
                              {String(evidence.created_at ?? '').replace('T', ' ').slice(0, 16)}
