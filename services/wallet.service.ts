...
import type { CreditCustomer } from './credit/credit.service';
...
export const walletService = {
  async listWalletsByBranch(branchId: string) {
    const { data, error } = await supabase
      .from('v_customer_wallets_summary')
      .select('*')
  ...
  async createWallet(input: {
    ...
    const { data, error } = await supabase.rpc('create_customer_wallet', {
  ...
  async rechargeWallet(input: {
    ...
    const { data, error } = await supabase.rpc('recharge_customer_wallet', {
  ...
  async applyWalletToSale(input: {
    ...
    const { data, error } = await supabase.rpc('apply_wallet_to_sale', {
      p_wallet_id: input.wallet_id,
