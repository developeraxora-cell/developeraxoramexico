...
export const creditService = {
  ...
  async listCustomerDocuments(customerId: string) {
    const { data, error } = await concreteDb
      .from('concrete_credit_customer_documents')
      .select('*')
      .eq('customer_id', customerId)
  ...
  async upsertCustomerDocument(input: {
    ...
    const { data: existing, error: existingError } = await concreteDb
      .from('concrete_credit_customer_documents')
      .select('id')
      .eq('customer_id', input.customer_id)
      .eq('slot', input.slot)
  ...
  async listPaymentEvidencesByPaymentIds(paymentIds: string[]) {
    ...
    const { data, error } = await concreteDb
      .from('concrete_credit_payment_evidences')
      .select('*')
  ...
  async listSaleEvidencesByTransactionIds(transactionIds: string[]) {
    ...
    const { data, error } = await concreteDb
      .from('concrete_inventory_transaction_evidences')
      .select('*')
