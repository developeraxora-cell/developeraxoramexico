...
export const catalogService = {
  ...
  async adjustProductStock(input: StockAdjustmentInput) {
    ...
    const { data, error } = await concreteDb.rpc('adjust_concrete_inventory_stock', {
    ...
    if (error) {
      ...
      if (code === '42883') {
        throw new Error('Falta la función de ajuste de stock de Concretera en la base de datos. Ejecute el script SQL de migración.');
      }
      if (code === 'PGRST202' || code === 'PGRST204') {
        throw new Error('No se encontró el endpoint RPC de ajuste de stock de Concretera. Verifique que la función exista en Supabase.');
      }
      if (message.toLowerCase().includes('adjust_concrete_inventory_stock')) {
        throw new Error('No se encontró la función adjust_concrete_inventory_stock en la base de datos.');
      }
      throw error;
    }
  ...
  async getProductMovementHistory(branchId: string, productId: string, limit = 120) {
    ...
    const { data: txItems, error: txItemsError } = await concreteDb
      .from('concrete_inventory_transaction_items')
      .select(`
        id,
        qty,
    ...
    const { data: adjustmentsData, error: adjustmentsError } = await concreteDb
      .from('concrete_inventory_stock_adjustments')
    ...
    if (adjustmentsError) {
      ...
      if (code === '42P01') {
        throw new Error('Falta la tabla concrete_inventory_stock_adjustments en la base de datos. Ejecute el script SQL de migración.');
      }
    ...
    const transactionRows: ProductMovementRow[] = (txItems ?? []).map((row: any) => {
      const transaction = row.concrete_inventory_transactions;
