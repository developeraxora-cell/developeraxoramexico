...
export const purchasesService = {
  ...
  async createPurchase(input: CreatePurchaseInput) {
    ...
    const { data: uoms, error: uomError } = await supabase
      .from('product_uoms')
      .select('id, factor_to_base, uom_id, uoms (code, name)')
      .in('id', uomIds);
    ...
    const { data: baseUoms, error: baseUomError } = await supabase
      .from('uoms')
      .select('id, code, name')
      .in('id', baseUomIds);
    ...
    (uoms ?? []).forEach((row: any) => {
      uomMap.set(String(row.id), {
        ...
        uom_code: row.uoms?.code ?? null,
        uom_name: row.uoms?.name ?? null,
      });
    });
    ...
    if (totalsProductIds.length > 0) {
      const { data: existingRows, error: stockError } = await supabase
  ...
  async deletePurchase(input: DeletePurchaseInput) {
    const { data, error } = await supabase.rpc('delete_inventory_purchase', {
